  ...
  return (
    <Modal centered
      ...
      <Modal.Header className="text-center" closeButton>
        <Modal.Title as="h3" id="about-modal-title">Register Individuals vs Businesses</Modal.Title>
      </Modal.Header>
      <Modal.Body>
...
const RegisterBusiness = ({ page, previous, next, isActive }) => {
  ...
  return (
    <Container fluid className={`main-content-container d-flex flex-column flex-grow-1 loaded ${page.replace('/', '')}`}>
      ...
      <p className="mb-4 text-info text-lg">{app.activeUser ? "We've gathered some information to see if you meet KYB guidelines. If you'd like to add, update or delete information, you can do so here." : "We need to gather some information to see if this business meets KYB guidelines."}</p>

      <p className="mb-4 text-info text-lg">To register a new end-user as a business instead of an individual, the following must be sent in the request:</p>
      ...
      <RegisterBusinessForm handle={app.settings.kybHandle} onSuccess={registerUser} onShowKybModal={(showKybModal) => setShowKybModal(showKybModal)} onConfirm={setConfirm}>
        ...
        {app.settings.preferredKybLevel && !app.activeUser && <Alert variant="primary" className="my-4">A wallet is automatically generated for you using the generateWallet() function upon registration.</Alert>}
