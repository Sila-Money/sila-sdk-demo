      ...
      {!member ? <Loader /> : <div className="loaded">
          ...
          {member.memberships.length !== 0 && <Card>
            ...
            <ListGroup variant="flush">
              {member.memberships.map((membership, index) => <ListGroup.Item variant="flush" key={index}>
                ...
                {membership.details && <p className="m-0 text-info text-sm">{membership.details}</p>}
              </ListGroup.Item>)}
            </ListGroup>
          </Card>}
        </>}
        ...
        {!location.pathname.includes('certify') && <>
          {showMemberForm && <>
            ...
            <p className="text-info text-lg mb-2">We've gathered some information to see if this business member meet KYC guidelines. If you'd like to add, update or delete information, you can do so here.</p>
          ...
          <p className="mt-3 mb-0 text-center"><Button variant="outline-light" className="text-info text-uppercase" onClick={() => history.goBack()} disabled={member.memberships.length === 0}>I'm Done</Button></p>
        </>}
